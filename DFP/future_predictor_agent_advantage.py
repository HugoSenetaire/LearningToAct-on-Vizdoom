...
class FuturePredictorAgentAdvantage(Agent):
    ...
    def make_net(self, input_images, input_measurements, input_actions, input_objectives, reuse=False):
        ...
        p_img_UNET = my_ops.flatten(my_ops.UNET(input_images,self.unet_params,'UNET',msra_coeff=0.9))
        # p_img_conv = tf.concat([p_img_conv,p_img_UNET
        p_img_fc = my_ops.fc_net(my_ops.flatten(p_img_conv), self.fc_img_params, 'p_img_fc', msra_coeff=0.9)
        p_seg_fc = my_ops.fc_net(p_img_UNET,self.fc_img_params,'p_unet_fc',msra_coeff=0.9)
        p_img_fc = tf.concat([p_img_fc,p_img_UNET],1)
        p_meas_fc = my_ops.fc_net(input_measurements, self.fc_meas_params, 'p_meas_fc', msra_coeff=0.9)
        # p_segm_conv = my_ops.RCNN(input_images, self.conv_params, 'p_img_conv',msra_coeff=0.9)
